...
from sdcm.argus_results import send_manager_benchmark_results_to_argus, submit_results_to_argus, BackupResult, \
    ReadResult
...
class ManagerBackupRestoreConcurrentTests(ManagerTestFunctionsMixIn):
    def report_to_argus(self, report_type: ReportType, data: dict, label: str):
        if report_type == ReportType.READ:
            table = ReadResult(sut_timestamp=mgmt.get_scylla_manager_tool(
                manager_node=self.monitors.nodes[0]).sctool.client_version_timestamp)
        elif report_type == ReportType.BACKUP:
            table = BackupResult(sut_timestamp=mgmt.get_scylla_manager_tool(
                manager_node=self.monitors.nodes[0]).sctool.client_version_timestamp)
        else:
    ...
    def run_read_stress_and_report(self, label):
        ...
        self.report_to_argus(ReportType.READ, read_stress_report, label)
    ...
    def test_backup_benchmark(self):
        ...
        backup_task = self.create_backup_and_report(mgr_cluster, "Backup times")
